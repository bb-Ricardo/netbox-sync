...
def test_get_role_normalizes_netbox_dict_form(inventory):
    # NetBox reports the role as {"value": ..., "label": ...}; get_role() returns the value
    _vm, nic = _vm_and_nic(inventory, "vm-a")
    ip = _add_ip(inventory, nic, role={"value": "anycast", "label": "Anycast"})
    assert ip.get_role() == "anycast"


def test_get_role_returns_plain_value(inventory):
    # an object this program created itself carries the plain string
    _vm, nic = _vm_and_nic(inventory, "vm-a")
    ip = _add_ip(inventory, nic, role="vip")
    assert ip.get_role() == "vip"


def test_get_role_is_none_when_unset(inventory):
    _vm, nic = _vm_and_nic(inventory, "vm-a")
    ip = _add_ip(inventory, nic)
    assert ip.get_role() is None
